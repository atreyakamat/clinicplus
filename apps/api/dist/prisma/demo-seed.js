...
async function main() {
    ...
    let demoDoctor, demoReceptionist, demoOwner;
    ...
    if (existingUsers.length >= 3) {
        [demoDoctor, demoReceptionist, demoOwner] = existingUsers.filter(u => ['doctor.demo@clinicos.com', 'receptionist.demo@clinicos.com', 'owner.demo@clinicos.com'].includes(u.email));
    ...
    const appointments = [];
    for (let i = 0; i < 200; i++) {
        const patient = faker_1.faker.helpers.arrayElement(actualPatients);
        appointments.push({
            organizationId: patient.organizationId,
            branchId: patient.branchId,
            patientId: patient.id,
            doctorId: demoDoctor.id,
            scheduledStart: faker_1.faker.date.between({ from: '2026-01-01', to: '2026-12-31' }),
            scheduledEnd: faker_1.faker.date.soon({ days: 30 }),
    ...
    for (const cons of prescriptionConsultations) {
        await prisma.prescription.create({
            data: {
                ...
                items: {
                    create: Array.from({ length: faker_1.faker.number.int({ min: 1, max: 3 }) }).map(() => ({
                        organizationId: cons.organizationId,
                        branchId: cons.branchId,
            ...
            data: {
                ...
                payments: {
                    create: {
                        ...
                        branchId: cons.branchId,
                        amount: subtotal,
                        paymentMethod: faker_1.faker.helpers.arrayElement(['CASH', 'UPI', 'CARD']),
                        paymentStatus: 'PAID'
                    }
                }
            }
    ...
    for (const patient of followUpPatients) {
        followUps.push({
            ...
            patientId: patient.id,
            doctorId: demoDoctor.id,
            scheduledDate: faker_1.faker.date.soon({ days: 30 }),
            status: 'PENDING'
        });
    }
