...
describe('AnalyticsController', () => {
  ...
  describe('getDoctorDashboard', () => {
    it('should call service with injected user context', async () => {
      jest.spyOn(service, 'getDoctorDashboard').mockResolvedValue({ stats: {}, chartData: [] } as any);
      
      const result = await controller.getDoctorDashboard(mockRequest);
      expect(result).toBeDefined();
      expect(service.getDoctorDashboard).toHaveBeenCalledWith('doc-1', 'org-1', 'branch-1');
    });
  });
});
