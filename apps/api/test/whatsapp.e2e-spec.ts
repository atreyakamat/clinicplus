...
describe('WhatsApp / Messaging (E2E) — Phase 12', () => {
  ...
  beforeAll(async () => {
    ...
    org = await prisma.organization.create({ data: { name: 'Msg Test', slug: `msg-${Date.now()}` } });
    branch = await prisma.branch.create({ data: { name: 'Msg Branch', organizationId: org.id } });
    user = await prisma.user.create({
      data: { email: `msg-${Date.now()}@t.com`, passwordHash: 'h', firstName: 'Msg', lastName: 'User', organizationId: org.id, branchId: branch.id },
    });
    patient = await prisma.patient.create({
      data: { firstName: 'Msg', lastName: 'Pat', phone: '5551000001', organizationId: org.id, branchId: branch.id },
    });
    ...
    it('should record communication in the database', async () => {
      await prisma.message.create({
        data: {
          ...
          channel: 'WHATSAPP',
          direction: 'OUTBOUND',
          messageBody: 'Your appointment is confirmed for tomorrow at 10:00 AM.',
        },
      });
