...
describe('Consultation (E2E) — Phase 8', () => {
  ...
  beforeAll(async () => {
    ...
    org = await prisma.organization.create({
      data: { name: 'Cons Test', slug: `cons-${Date.now()}` },
    });
    branch = await prisma.branch.create({
      data: { name: 'Cons Branch', organizationId: org.id },
    });
    doctor = await prisma.user.create({
      data: {
        email: `dr-c-${Date.now()}@t.com`,
        passwordHash: 'h',
        firstName: 'DrC',
        lastName: 'Test',
  ...
  describe('Consultation Management', () => {
    ...
    it('should get consultation by ID', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/consultations/${consultId}`)
        .set('Authorization', `Bearer ${token}`);
      expect(res.status).toBeDefined();
      const body = res.body.data || res.body;
    ...
    it('should update consultation', async () => {
      const res = await request(app.getHttpServer())
        .patch(`/api/v1/consultations/${consultId}`)
        .set('Authorization', `Bearer ${token}`)
    ...
    it('should list consultations filtered by patient', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/consultations?patientId=${patient.id}`)
        .set('Authorization', `Bearer ${token}`);
      expect(res.status).toBeDefined();
      const body = res.body.data || res.body;
