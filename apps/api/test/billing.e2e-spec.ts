...
describe('Billing (E2E) — Phase 10', () => {
  ...
  let org: any; let branch: any; let user: any; let patient: any; let token: string;
  ...
  beforeAll(async () => {
    ...
    org = await prisma.organization.create({ data: { name: 'Bill Test', slug: `bill-${Date.now()}` } });
    branch = await prisma.branch.create({ data: { name: 'Bill Branch', organizationId: org.id } });
    user = await prisma.user.create({
      data: { email: `bill-${Date.now()}@t.com`, passwordHash: 'h', firstName: 'Bill', lastName: 'User', organizationId: org.id, branchId: branch.id },
    });
    patient = await prisma.patient.create({
      data: { firstName: 'Bill', lastName: 'Pat', phone: '5550800001', organizationId: org.id, branchId: branch.id },
    });
  ...
  describe('Payments', () => {
    ...
    beforeEach(async () => {
      const inv = await prisma.invoice.create({
        data: { patientId: patient.id, organizationId: org.id, branchId: branch.id, invoiceNumber: `INV-PAY-${Date.now()}`, total: 200 },
      });
      invId = inv.id;
    });
    ...
    it('should add partial payment', async () => {
      const inv = await prisma.invoice.create({
        data: { patientId: patient.id, organizationId: org.id, branchId: branch.id, invoiceNumber: `INV-PART-${Date.now()}`, total: 300 },
      });
      const res = await request(app.getHttpServer())
  ...
  describe('Invoice Retrieval', () => {
    ...
    beforeEach(async () => {
      const inv = await prisma.invoice.create({
        data: { patientId: patient.id, organizationId: org.id, branchId: branch.id, invoiceNumber: `INV-GET-${Date.now()}`, total: 75 },
      });
      invId = inv.id;
    });
    ...
    it('should download invoice as PDF', async () => {
      const res = await request(app.getHttpServer())
        .get(`/api/v1/invoices/${invId}/download`).set('Authorization', `Bearer ${token}`);
      if (res.status === 200) {
