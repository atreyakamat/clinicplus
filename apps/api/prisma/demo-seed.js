...
async function main() {
  ...
  for (let i = 0; i < 200; i++) {
    ...
    appointments.push({
      ...
      scheduledEnd: new Date(faker.date.between({ from: '2026-01-01', to: '2026-12-31' }).getTime() + 30 * 60000), // 30 minutes later
  ...
  for (const patient of followUpPatients) {
    followUps.push({
      ...
      patientId: patient.id,
      doctorId: demoDoctor.id,
      scheduledDate: faker.date.soon({ days: 30 }), // Approximate future date (within 30 days)
      status: 'PENDING'
    });
  }
